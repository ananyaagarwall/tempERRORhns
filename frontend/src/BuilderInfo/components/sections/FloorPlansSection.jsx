...
import { pickProjectImage, pickFloorPlanPreview } from '../../../utils/projectImageUtils';
    ...
    <div className="bg-white rounded-xl xs:rounded-2xl border border-gray-200 p-4 xs:p-5 sm:p-6 md:p-8 shadow-sm">
      <div className="flex flex-col xs:flex-row xs:justify-between xs:items-start sm:flex-row sm:justify-between sm:items-center">
        ...
        <button className="flex items-center text-blue-600 hover:text-blue-700 font-sans-serif text-xs xs:text-m sm:text-base transition-colors mt-2 xs:mt-0">
          View more
          <svg className="ml-1 w-3 h-3 xs:w-4 xs:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const formatBhkInfo = (project) => {
  const configs = Array.isArray(project?.unit_configs) ? project.unit_configs : [];
  const labels = [...new Set(configs.map((row) => row.bhk_type).filter(Boolean))];
  if (labels.length > 0) return labels.join(' · ');
  if (Array.isArray(project?.configuration) && project.configuration.length > 0) {
    return project.configuration.map((item) => item.type || item.bhk_type).filter(Boolean).join(' · ');
  }
  ...
  >
    ...
    <div className="relative h-full flex flex-col justify-between p-3 xs:p-4 text-white">
      ...
      <div className="bg-black bg-opacity-50 rounded-lg p-2 xs:p-3 backdrop-blur-sm">
        ...
        {floorPlan.price && <p className="text-sm xs:text-base font-var(--hns-sans) text-yellow-300 mb-2">{floorPlan.price}</p>}
        {floorPlan.location && (
      ...
      try {
        ...
        const cards = await Promise.all(
          (Array.isArray(projects) ? projects : []).map(async (project) => {
            ...
            const media = await fetchEntityMedia('project', project.id, 'floor_plan');
            ...
            return {
              ...
              subtitle: property.Project_Status || project.property_status || project.status || 'View floor plans',
              image,
              projectName: property.Location || project.location || 'Location on request',
              bhkInfo: formatBhkInfo(project),
              price: property.Price_Starting_From || property.Pricing || project.price_range || '',
              location: project.full_address || property.Address || project.location || '',
    ...
    <div>
      <div className="my-6 xs:my-7 sm:my-8">
        ...
        <div className="block lg:hidden">
          <div className="overflow-x-auto hide-scrollbar" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none', padding: '12px 0 20px 0', scrollSnapType: 'x mandatory' }}>
            <style>{`
              .hide-scrollbar::-webkit-scrollbar { display: none; }
              .hide-scrollbar { -ms-overflow-style: none; scrollbar-width: none; }
            `}</style>
            <div className="floorplans-mobile-row flex gap-3 xs:gap-4" style={{ width: 'max-content', padding: '0 16px', scrollSnapType: 'x mandatory', WebkitOverflowScrolling: 'touch', scrollBehavior: 'smooth' }}>
              {floorPlans.map((floorPlan, idx) => (
                <div
                  ...
                  style={{ width: '260px', height: '300px', border: '2px solid #1e3a8a', borderRadius: '12px', scrollSnapAlign: 'start', overflow: 'hidden', backgroundColor: 'white' }}
                  onClick={() => openProperty(floorPlan)}
                >
                  <FloorPlanCard floorPlan={floorPlan} onClick={() => openProperty(floorPlan)} />
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="relative p-4 xs:p-5 sm:p-6 hidden lg:block" style={{ border: '2px solid #1e3a8a', borderRadius: '8px', marginTop: '24px' }}>
        <button onClick={handlePrevious} className="absolute left-0 top-1/2 -translate-y-1/2 z-30 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md" aria-label="Previous" style={{ left: '20px', zIndex: 40 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M15 18L9 12L15 6" stroke="#223A5F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
        <button onClick={handleNext} className="absolute right-0 top-1/2 -translate-y-1/2 z-30 bg-white rounded-full w-10 h-10 flex items-center justify-center shadow-md" aria-label="Next" style={{ right: '20px', zIndex: 40 }}>
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M9 18L15 12L9 6" stroke="#223A5F" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </button>
        ...
        <div className="flex items-center justify-center relative" style={{ minHeight: '320px', perspective: '1000px' }}>
          {floorPlans.map((floorPlan, idx) => {
            ...
            if (offset === -2) { translateX = -320; scale = 0.7; zIndex = 5; rotateY = 15; }
            else if (offset === -1) { translateX = -160; scale = 0.85; zIndex = 15; rotateY = 10; }
            else if (offset === 1) { translateX = 160; scale = 0.85; zIndex = 15; rotateY = -10; }
            else if (offset === 2) { translateX = 320; scale = 0.7; zIndex = 5; rotateY = -15; }
